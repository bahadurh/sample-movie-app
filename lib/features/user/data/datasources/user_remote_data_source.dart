...
class UserRemoteDataSourceImpl
  ...
  Future<bool> createUser(User user) async {
    return await makePostRequest(dioClient.post(ApiConfig.users, data: user));
  }

  @override
  Future<bool> updateUser(User user) async {
    return await makePutRequest(
        dioClient.put("${ApiConfig.users}/${user.id}", data: user));
  }

  @override
  Future<bool> deleteUser(User user) async {
    return await makeDeleteRequest(
        dioClient.delete("${ApiConfig.users}/${user.id}"));
  }
